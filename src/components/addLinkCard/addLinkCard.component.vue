  ...
  div.card.card-body.custom-border-shadow
    div.container-md.px-0.px-md-3
      ...
      form(@submit.prevent="saveBtnHandler")
        autocomplete-form(v-model="model.courseTitle" :inputClass="!validations.course ? 'custom-validation-error' : ''" placeholder="Select course" :autocomplete="$store.getters.getCourses" :autocompleteItemName="courseNameHandler" :onAutocompleteItemClick="onCourseClickHandler" :onAutocompleteInput="onSearchInputHandler" :onAutocompleteClick="onSearchClickHandler" :onClearIconClick="clearIconClickHandler")
